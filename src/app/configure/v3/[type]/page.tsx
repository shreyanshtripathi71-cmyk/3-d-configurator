...
export default function ConfiguratorPage() {
  ...
  const [drawerOpenV3, setDrawerOpenV3] = useState(false);
  ...
  /* OpenSpec v23.4 viewer-toolbar state */
  const [viewerMode, setViewerMode] = useState<'2d' | 'wireframe'>('wireframe');
  const [viewerSide, setViewerSide] = useState<'exterior' | 'interior' | 'split'>('exterior');
  ...
  /* ════════════════════════════════════════════════════════════
     V3 — NANOKAD CLASSIC (May 1 deployment recreation)
     Recreates the design that was live on Vercel before today,
     2-col: viewer + action bar (left) + tab inspector (right).
     ════════════════════════════════════════════════════════════ */
  ...
  // Aliases mapping new-variant state to classic design naming
  const activeTab = activeStep;
  const setActiveTab = setActiveStep;
  const sectionList = SECTION_CONFIG;

  void drawerOpenV3; void setDrawerOpenV3; void dimsVisible;
  void viewerMode; void setViewerMode; void viewerSide; void setViewerSide;
  void showDims; void setShowDims; void showHardware; void setShowHardware;
  void showNotes; void setShowNotes; void sectionList;

  const cellWidthInches = (() => {
    if (!selectedCell) return config.frameWidth;
    const rowCfg = config.grid.rowConfigs?.find((rc) => rc.row === selectedCell.row);
    const rowH = rowCfg?.horizontalCount || config.grid.horizontalCount;
    return Math.round((config.frameWidth / rowH) * 10) / 10;
  })();

  return (
    <div className={styles.scope}>
      {/* ════ TOPBAR — exact OpenSpec html replica (64px, glass) ════ */}
      <header className={styles.osNav}>
        <div className={styles.osNavLeft}>
          <Link href="/" className={styles.osNavBrand} aria-label="OpenSpec home">
            <svg width="28" height="28" viewBox="0 0 20 20" fill="none">
              <rect x="2" y="2" width="16" height="16" rx="3.6" fill="#2563EB" />
          ...
          <nav className={styles.osNavMenu} id="topNav">
            <button className={styles.osNavItem} type="button">Dashboard</button>
            <button className={`${styles.osNavItem} ${styles.osNavItemActive}`} type="button">Quotes</button>
            <button className={styles.osNavItem} type="button">Orders</button>
            <button className={styles.osNavItem} type="button">Customers</button>
            <button className={styles.osNavItem} type="button">Calendar</button>
            <button className={styles.osNavItem} type="button">Commissions</button>
            <button className={styles.osNavItem} type="button">
              <svg width="15" height="15" viewBox="0 0 16 16" fill="none" style={{ marginRight: 5, display: 'inline-block', verticalAlign: -2 }}>
        ...
        <div className={styles.osNavRight}>
          <Link href={`/configure/${typeId}`} className={styles.osNavTip} title="Variant 1 (CPQ classic)">Variant 1</Link>
          <Link href={`/configure/v2/${typeId}`} className={styles.osNavTip} title="Variant 2 (OpenSpec showcase)">Variant 2</Link>
          ...
          <button className={styles.osNavUser} type="button" title="Account menu">
      ...
      {/* ════ BREADCRUMB row (project context + save status + cart) ════ */}
      <div className={styles.osBcRow}>
        <div className={styles.osBcLeftRow}>
          <Link href="/windows" className={styles.osBcBackBtn}>
          ...
          <span className={styles.osBcCrumb}>{windowType.label}</span>
          <span className={styles.osBcDivider}>/</span>
          <span className={styles.osBcCurrent}>Configure</span>
          <span className={styles.osBcMeta}>
            <span className={styles.osBcMetaLabel}>Project</span>
            <input
              className={styles.osBcMetaInput}
          ...
          <span className={styles.osBcMeta}>
            <span className={styles.osBcMetaLabel}>PO #</span>
            <input
              className={styles.osBcMetaInput}
              placeholder="—"
              value={poRef}
          ...
          <span className={styles.osBcMeta}>
            <span className={styles.osBcMetaLabel}>Customer</span>
            <input
              className={styles.osBcMetaInput}
        ...
        <div className={styles.osBcRightRow}>
          ...
          <button className={styles.osIconBtn} title="Save draft" onClick={handleSave} type="button">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
              <path d="M5 4h11l3 3v13H5z" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" />
              <path d="M8 4v5h7" stroke="currentColor" strokeWidth="1.7" strokeLinejoin="round" />
              <path d="M9 13h6M9 16h4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
            </svg>
          </button>
          <button className={styles.osIconBtn} title="View quote" onClick={() => setDrawerOpenV3(true)} type="button">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
              <path d="M3 6h2l2 11h11l2-8H7" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" />
              <circle cx="9" cy="20" r="1.4" fill="currentColor" />
              <circle cx="17" cy="20" r="1.4" fill="currentColor" />
            </svg>
            {lines.length > 0 && <span className={styles.osIconBtnDot} />}
          </button>
          <button className={styles.osDealerPill} type="button" title={`${DEALER.name} · ${DEALER.tier}`}>
            <span className={styles.osDealerAvatar}>{DEALER.initials}</span>
            <span className={styles.osDealerName}>{DEALER.name}</span>
            <span className={styles.osDealerTier}>Pro</span>
          </button>
          <button className={styles.osQuoteCta} type="button" onClick={() => setDrawerOpenV3(true)}>
            <span className={styles.osQuoteCtaId}>{quoteNumber}</span>
            <span className={styles.osQuoteCtaCount}>{lines.length}</span>
            <span className={styles.osQuoteCtaTotal}>${(quoteSubtotal + quoteTax).toLocaleString('en-US', { maximumFractionDigits: 0 })}</span>
          </button>
        </div>
      </div>

      {/* ════ MAIN APP — 2 col: viewer column + inspector card ════ */}
      <main className={styles.app}>
        {/* LEFT — viewer (smaller) + action bar */}
          ...
          <div className={styles.viewerCard}>
            ...
            <div className={styles.viewerBottomBar}>
              <div className={styles.controlsCluster}>
                <button className={styles.controlIcon} onClick={() => controlsRef.current?.zoomOut()} title="Zoom out" type="button">
                ...
                <button className={styles.controlIcon} onClick={() => controlsRef.current?.zoomIn()} title="Zoom in" type="button">
                ...
                <div className={styles.dpadCluster}>
                  <button className={styles.dpadBtn} data-pos="top" onClick={() => controlsRef.current?.rotateUp()} title="Tilt up" type="button">
                    <svg width="11" height="11" viewBox="0 0 16 16" fill="none"><path d="M4 10l4-4 4 4" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round"/></svg>
                  </button>
                  <button className={styles.dpadBtn} data-pos="left" onClick={() => controlsRef.current?.rotateLeft()} title="Rotate left" type="button">
                    <svg width="11" height="11" viewBox="0 0 16 16" fill="none"><path d="M10 12L6 8l4-4" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round"/></svg>
                  </button>
                  <button className={styles.dpadBtn} data-pos="right" onClick={() => controlsRef.current?.rotateRight()} title="Rotate right" type="button">
                    <svg width="11" height="11" viewBox="0 0 16 16" fill="none"><path d="M6 12l4-4-4-4" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round"/></svg>
                  </button>
                  <button className={styles.dpadBtn} data-pos="bottom" onClick={() => controlsRef.current?.rotateDown()} title="Tilt down" type="button">
                    <svg width="11" height="11" viewBox="0 0 16 16" fill="none"><path d="M4 6l4 4 4-4" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round"/></svg>
                  </button>
                </div>
                <span className={styles.controlSep} />
                <button className={styles.controlIcon} onClick={() => controlsRef.current?.resetView()} title="Reset view" type="button">
                ...
                <button className={styles.controlIcon} onClick={() => setIsFullscreen((v) => !v)} title="Fullscreen" type="button">
              ...
              <div className={styles.dimensionReadout}>
                <div className={styles.dimReadoutItem}>
                  <span className={styles.dimReadoutLabel}>Width</span>
                  <span className={styles.dimReadoutValue}>{config.frameWidth}″</span>
                </div>
                <span className={styles.dimReadoutSep} />
                <div className={styles.dimReadoutItem}>
                  <span className={styles.dimReadoutLabel}>Height</span>
                  <span className={styles.dimReadoutValue}>{config.frameHeight}″</span>
                </div>
                <span className={styles.dimReadoutSep} />
                <div className={styles.dimReadoutItem}>
                  <span className={styles.dimReadoutLabel}>U-factor</span>
                  <span className={styles.dimReadoutValue}>{energyRatings?.uFactorIP ?? '0.27'}</span>
                </div>
                <span className={styles.dimReadoutSep} />
                <div className={styles.dimReadoutItem}>
                  <span className={styles.dimReadoutLabel}>SHGC</span>
                  <span className={styles.dimReadoutValue}>{energyRatings?.shgc ?? '0.28'}</span>
                </div>
              </div>
            </div>
          </div>

          {/* Action bar — SKU, lead time, price, qty, add-to-quote */}
          <div className={styles.actionBar}>
            <div className={styles.actionBarMetaRow}>
              <span className={styles.skuPlate}>
                <svg width="11" height="11" viewBox="0 0 16 16" fill="none"><rect x="2" y="3" width="12" height="10" rx="1" stroke="currentColor" strokeWidth="1.4"/><path d="M5 6v4M8 6v4M11 6v4" stroke="currentColor" strokeWidth="1.4"/></svg>
                ...
                <button
                  ...
                  title={skuCopied ? 'Copied!' : 'Copy SKU'}
                  type="button"
                >
                  <svg width="11" height="11" viewBox="0 0 16 16" fill="none"><rect x="4" y="4" width="9" height="9" rx="1" stroke="currentColor" strokeWidth="1.4"/><path d="M3 11V3h8" stroke="currentColor" strokeWidth="1.4"/></svg>
                </button>
              </span>
              <span className={styles.leadBadge}>
                <svg width="10" height="10" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="6" stroke="currentColor" strokeWidth="1.4"/><path d="M8 5v3l2 1" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/></svg>
                {leadTime}
              </span>
              <span className={styles.statusBadge}>
                {orderStatus === 'draft' ? 'Draft' : orderStatus === 'quoted' ? 'Quoted' : 'Submitted'}
              </span>
            </div>

            <div className={styles.actionBarMain}>
              <div className={styles.actionBarPriceCol}>
                <div className={styles.actionBarPriceMain}>
                  ${tieredUnitPrice.toLocaleString('en-US', { maximumFractionDigits: 0 })}
                  <span className={styles.actionBarPriceUnit}>/unit</span>
                </div>
                <div className={styles.actionBarPriceListLine}>
                  <s>${priceData.total.toLocaleString('en-US', { maximumFractionDigits: 0 })}</s>
                  <span className={styles.actionBarSavingsBadge}>Save {(effectiveDiscount * 100).toFixed(0)}%</span>
                  <button
                    className={styles.tierToggle}
                    onClick={() => setTiersExpanded((v) => !v)}
                    type="button"
                  >
                    {tiersExpanded ? 'Hide volume tiers' : 'Volume pricing'}
                  </button>
                </div>
              </div>

              <div className={styles.actionBarActionsCol}>
                <div className={styles.actionBarQty}>
                  <span className={styles.actionBarQtyLabel}>Qty</span>
                  <Stepper
                    value={config.quantity}
                    onChange={(v) => quickUpdate({ quantity: v })}
                    min={1}
                    max={9999}
                    compact
                  />
                </div>
                <button className={styles.actionBarCta} onClick={addToQuote} type="button">
                  Add to quote
                  <span className={styles.actionBarCtaSep} />
                  <span className={styles.actionBarCtaTotal}>
            ...
            {tiersExpanded && (
              <div className={styles.actionBarTiers}>
                {TIER_BREAKS.map((t) => {
                  const isActive = currentTier.label === t.label;
                  const eff = Math.min(0.3, t.discount + DEALER.accountDiscount);
                  const tierPrice = priceData.total * (1 - eff);
                  return (
                    <div key={t.label} className={`${styles.actionBarTier} ${isActive ? styles.actionBarTierActive : ''}`}>
                      <div className={styles.actionBarTierLeft}>
                        <span className={styles.actionBarTierRange}>{t.label}</span>
                        <span className={styles.actionBarTierPrice}>${tierPrice.toLocaleString('en-US', { maximumFractionDigits: 0 })}</span>
                      </div>
                      <span className={styles.actionBarTierDisc}>−{(eff * 100).toFixed(0)}%</span>
                    </div>
                  );
                })}
              </div>
        ...
        {/* RIGHT — inspector card with horizontal tabs (Layout / Frame / Glass / Cell / Grilles / Energy / Spec) */}
        <aside className={styles.inspector}>
          <header className={styles.productHeadSlim}>
            <div className={styles.productHeadSlimLeft}>
              <h2 className={styles.productHeadSlimTitle}>{windowType.label}</h2>
              <span className={styles.productHeadSlimType}>{typeId.toUpperCase()}</span>
            </div>
          ...
          <nav className={styles.tabs}>
            {SECTION_CONFIG.map((sec) => (
              ...
              >
                ...
                {sec.shortLabel}
                {tabHasCustomization[sec.id as keyof typeof tabHasCustomization] && <span className={styles.tabBadge} />}
              </button>
            ))}
          </nav>
          ...
          <div className={styles.inspectorBody}>
              ...
              <LayoutTab
                ...
                onChangeRowHeight={(rowIdx, newH) => {
                  setConfig((prev) => ({
                    ...
                    grid: {
                      ...prev.grid,
                      cells: prev.grid.cells.map((c) => c.row === rowIdx ? { ...c, height: newH } : c),
                    },
                    frameHeight: prev.grid.cells.reduce((sum, c, idx, arr) => {
                      const r = c.row;
                      const seenRows = new Set(arr.slice(0, idx).map((cc) => cc.row));
                      if (seenRows.has(r)) return sum;
                      return sum + (r === rowIdx ? newH : c.height);
                    }, 0),
                  }));
                }}
            ...
            {activeStep === 'glass' && (
              <GlassTab config={config} onUpdate={updateConfig} />
            )}
            {activeStep === 'cell' && selectedCell && (
              <CellTab
                cell={selectedCell}
                typeId={selectedCell.windowType}
            ...
            {activeStep === 'energy' && selectedCell && energyRatings && (
          ...
          <footer className={styles.inspectorFooter}>
            <div className={styles.footerLeft}>
              <button
                ...
                type="button"
                onClick={() => {
                  const i = SECTION_CONFIG.findIndex((s) => s.id === activeStep);
                  if (i > 0) setActiveStep(SECTION_CONFIG[i - 1].id);
                }}
              >
                <svg width="11" height="11" viewBox="0 0 16 16" fill="none"><path d="M10 12 6 8l4-4" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/></svg>
                Previous
            ...
            <div className={styles.footerRight}>
              <button
                ...
                onClick={() => {
                  const i = SECTION_CONFIG.findIndex((s) => s.id === activeStep);
                  if (i < SECTION_CONFIG.length - 1) setActiveStep(SECTION_CONFIG[i + 1].id);
                }}
              >
                Next
                <span className={styles.footerKey}>→</span>
                <svg width="11" height="11" viewBox="0 0 16 16" fill="none"><path d="M6 12l4-4-4-4" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/></svg>
              </button>
            </div>
          </footer>
        </aside>
      ...
      {drawerOpenV3 && (
        <>
          <div className={styles.drawerOverlay} onClick={() => setDrawerOpenV3(false)} />
          <aside className={styles.drawer} role="dialog" aria-modal="true">
            <header className={styles.drawerHead}>
              <div className={styles.drawerHeadInfo}>
                <h3 className={styles.drawerTitle}>Quote {quoteNumber}</h3>
                <div className={styles.drawerSub}>{quoteUnits} units · {lines.length} lines</div>
              </div>
              <button className={styles.drawerClose} onClick={() => setDrawerOpenV3(false)} type="button">
            ...
            <section className={styles.drawerProject}>
              <div className={styles.drawerProjectRow}>
                <span className={styles.drawerProjectLabel}>Project</span>
                <input className={styles.drawerProjectInput} value={projectName} onChange={(e) => setProjectName(e.target.value)} />
              </div>
              <div className={styles.drawerProjectRow}>
                <span className={styles.drawerProjectLabel}>Customer</span>
                <input className={styles.drawerProjectInput} value={customerName} onChange={(e) => setCustomerName(e.target.value)} />
              </div>
              <div className={styles.drawerProjectRow}>
                <span className={styles.drawerProjectLabel}>Email</span>
                <input className={styles.drawerProjectInput} value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)} />
              </div>
              <div className={styles.drawerProjectRow}>
                <span className={styles.drawerProjectLabel}>Phone</span>
                <input className={styles.drawerProjectInput} value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} />
              </div>
              <div className={styles.drawerProjectRow}>
                <span className={styles.drawerProjectLabel}>Address</span>
                <input className={styles.drawerProjectInput} value={customerAddress} onChange={(e) => setCustomerAddress(e.target.value)} />
              </div>
              <div className={styles.drawerProjectRow}>
                <span className={styles.drawerProjectLabel}>Install date</span>
                <input className={styles.drawerProjectInput} type="date" value={installDate} onChange={(e) => setInstallDate(e.target.value)} />
              </div>
            </section>

            {lines.length === 0 ? (
              <div className={styles.drawerEmpty}>
                <span className={styles.drawerEmptyIcon}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M3 6h2l2 11h11l2-8H7" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"/></svg>
                </span>
                <div className={styles.drawerEmptyTitle}>No lines yet</div>
                <div className={styles.drawerEmptyText}>Configure a unit then click <strong>Add to quote</strong> to add it here.</div>
              </div>
            ) : (
              <div className={styles.drawerLines}>
                {lines.map((line) => (
                  <div key={line.id} className={styles.drawerLine}>
                    <div className={styles.drawerLineMark} style={{ background: line.thumbColor }} />
                    <div className={styles.drawerLineInfo}>
                      <div className={styles.drawerLineTitle}>{line.title}</div>
                      <div className={styles.drawerLineMeta}>{line.qty} × ${line.unitPrice.toLocaleString('en-US', { maximumFractionDigits: 0 })}</div>
                    </div>
                    <div className={styles.drawerLineRight}>
                      <span className={styles.drawerLinePrice}>${line.lineTotal.toLocaleString('en-US', { maximumFractionDigits: 0 })}</span>
                      <button className={styles.drawerLineRemove} onClick={() => setLines((ls) => ls.filter((l) => l.id !== line.id))} type="button">
            ...
            <footer className={styles.drawerFoot}>
              <div className={styles.drawerSummaryRow}>
                <span>Subtotal</span><strong>${quoteSubtotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong>
              </div>
              <div className={styles.drawerSummaryRow}>
                <span>Tax (8%)</span><strong>${quoteTax.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong>
              </div>
              <div className={styles.drawerSummaryDiv} />
              <div className={styles.drawerTotalRow}>
                ...
                <span className={styles.drawerTotalValue}>
                  ${(quoteSubtotal + quoteTax).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </span>
              </div>
              <div className={styles.drawerActions}>
                <button className={styles.drawerActionSecondary} onClick={handleSave} type="button">Save draft</button>
                <button
      ...
      {toast && (
        <div className={styles.toast}>
          <span className={styles.toastIcon}>
            <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M4 8l3 3 5-7" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>
          </span>
          <div>
            <div className={styles.toastStrong}>{toast.msg}</div>
            {toast.sub && <div style={{ fontSize: 11.5, color: '#64748B' }}>{toast.sub}</div>}
          </div>
        </div>
      )}
      ...
      {/* Hidden constants (here so they don't trigger unused-var warnings) */}
      <span hidden>{cellWidthInches}</span>
    </div>
  );
}
